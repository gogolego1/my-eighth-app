...
def _run_web_search(claim: str, reason: str):
    """키워드 추출 → 뉴스·신문기사 검색 → fallback 포함 JSON 변환"""

    # ── 키워드 추출: ai_analysis에서 가져오거나 이유 문장에서 직접 추출 ──
    ana      = st.session_state.ai_analysis or {}
    kw_list  = ana.get("keywords", [])

    # 키워드가 없으면 이유에서 명사 추출 (간단히 2~4글자 단어 우선)
    if not kw_list:
        words   = re.findall(r'[가-힣]{2,6}', reason)
        stop    = {"때문에","하기가","위해서","그리고","이유는","있기","없기","이다","이기","하면","에서","으로","이라","있어","없어"}
        kw_list = [w for w in words if w not in stop][:4]

    kw_str = " ".join(kw_list[:4])  # 검색어용 키워드 (4개 이내)

    # ── STEP 1: 뉴스·신문기사 Google Search ──
    search_prompt = f"""다음 키워드와 관련된 최신 뉴스 기사나 신문 기사를 Google에서 검색해주세요.
...
[검색 조건]
- 뉴스 기사 또는 신문 기사만 찾아주세요 (블로그, 광고, 쇼핑 제외)
- 각 기사의 아래 정보를 최대한 자세히 알려주세요:
  1. 기사 제목 (정확하게)
  2. 언론사 이름과 날짜
  3. 기사 핵심 내용 요약 (3~4문장, 구체적으로)
  4. 기사에서 가장 중요한 사실·발언 1문장 (따옴표 포함)
  5. 기사 URL
- 학생 이유를 뒷받침하거나 반박하는 기사를 각각 찾아주세요
- 총 4~6개 기사를 찾아주세요
- 출처를 반드시 명확히 밝혀주세요"""

    with st.spinner("🔍 관련 뉴스·신문 기사를 검색하고 있어요..."):
        raw_text, sources = call_gemini_with_search(search_prompt)

    if raw_text.startswith("__NO_KEY__"):
        render_no_key_warning(); return
    if raw_text.startswith("__ERROR__"):
        st.session_state.search_results = _fallback_from_sources(sources, claim)
        st.session_state.search_error   = "" if st.session_state.search_results else str(raw_text)
        return

    # ── STEP 2: 검색 원문 → JSON 변환 ──
    convert_prompt = f"""아래는 뉴스·신문 기사 검색 결과입니다.
각 기사를 초등학교 6학년 학생이 토론 근거로 활용할 수 있도록 정리해주세요.

[검색 결과 원문]
{raw_text[:3000]}

[그라운딩 출처 목록]
{json.dumps(sources, ensure_ascii=False)[:1000]}

[변환 규칙]
- title: 기사 제목 (원문 그대로)
- summary: 어려운 말을 쉽게 풀어서 2~3문장으로 요약 (초등학생 눈높이)
- evidence: 기사에서 근거로 쓸 수 있는 핵심 문장 1개
  → 반드시 구체적인 사실·발언·수치가 담긴 문장이어야 함
  → 예: "해양수산부에 따르면 국내 잘피 숲의 30%가 사라졌다"
- why: 왜 이 문장이 근거가 되는지 1문장 (초등학생 언어)
  → 예: "정부가 직접 발표한 사실이기 때문에 믿을 수 있어요"
  → 예: "실제 신문에서 보도된 사건이라 내 주장의 증거가 돼요"
- evidence_type: 뉴스기사
- source_label: 언론사명 + 날짜 (예: "한겨레, 2025.03.12")
- side: pro(학생 주장 지지) / con(반대) / neutral(중립)
- url: 그라운딩 출처 목록에서 가장 관련된 URL 연결

내용이 불분명한 기사는 제외하고, 내용이 명확한 기사만 포함하세요.
코드블록(```) 없이 JSON 배열만 출력:
[
  {{
    "title": "기사 제목",
    "summary": "초등학생용 쉬운 요약 2~3문장",
    "evidence": "근거로 쓸 수 있는 핵심 문장 1개",
    "why": "왜 근거가 되는지 1문장",
    "evidence_type": "뉴스기사",
    ...
    with st.spinner("📰 기사 내용을 쉽게 정리하고 있어요..."):
        convert_result = call_gemini_json(convert_prompt, "")

    # ── 파싱 시도 ──
    items = _parse_json_results(convert_result)

    # ── URL 보완: grounding sources에서 매칭 ──
    for i, item in enumerate(items):
        if not item.get("url") and i < len(sources):
            item["url"] = sources[i].get("url", "")

    # ── fallback: JSON 파싱 실패 시 grounding sources로 최소 카드 생성 ──
    if not items and sources:
        items = _fallback_from_sources(sources, claim)
        st.session_state.search_error = "기사 내용 파싱이 불완전해 출처만 표시합니다."
    else:
...
def _fallback_from_sources(sources: list, claim: str) -> list:
    """grounding sources만 있을 때 최소한의 카드 생성"""
    items = []
    for s in sources:
        url   = s.get("url", "")
        ...
        source_label = domain.group(1) if domain else "출처 불명"
        items.append({
            "title":         title,
            "summary":       "기사 전문을 확인하려면 아래 링크를 클릭해보세요.",
            "evidence":      "기사 내용을 직접 읽고 근거 문장을 찾아봐요.",
