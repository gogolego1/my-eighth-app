...
def _run_web_search(claim: str, reason: str):
    """
    2단계 완전 분리:
    STEP 1. Google Search 그라운딩 → 자유 텍스트로 기사 정보 수집 (JSON 파싱 X)
    STEP 2. 수집된 텍스트 → 별도 call_gemini_json으로 JSON 변환
    """
    ...
    # ══ STEP 1: 그라운딩으로 자유 텍스트 수집 ══════════════════════
    # ⚠ 절대 JSON 요청 금지 — 자유로운 텍스트로 받기
    grounding_query = f"""
다음 키워드와 관련된 한국 뉴스 기사를 찾아서 소개해줘.

키워드: {kw_str}
토론 주제: {TOPIC}
학생 주장: {claim}

각 기사마다 아래 내용을 알려줘 (자유롭게 서술):
1. 기사 제목
2. 언론사 이름과 날짜
3. 기사 핵심 내용 (3~4문장)
4. 가장 중요한 사실이나 수치 한 문장
5. 기사 URL (알면 적어줘)

총 4~5개 기사를 찾아줘.
뉴스·신문 기사만 포함하고, 블로그나 광고는 제외해줘.
"""
    with st.spinner("🔍 Gemini가 관련 기사를 검색하고 있어요..."):
        raw_text, grounding_sources = call_gemini_with_search(grounding_query)

    if raw_text.startswith("__NO_KEY__"):
        render_no_key_warning(); return

    # 그라운딩 자체가 에러면 지식 기반으로 fallback
    ...
    # 그라운딩 출처 정보 정리
    source_info = "\n".join([
        f"- 제목: {s.get('title','')}, URL: {s.get('url','')}"
        for s in grounding_sources
    ]) if grounding_sources else "(출처 없음)"

    # ══ STEP 2: 수집된 텍스트 → JSON 변환 ════════════════════════
    # 이 단계에서 비로소 JSON 요청 — 그라운딩과 완전히 분리
    convert_sys = f"""당신은 초등학교 6학년 학생의 토론을 돕는 선생님입니다.
아래에 뉴스 기사 정보가 있습니다. 이것을 학생이 근거로 활용할 수 있도록 정리해주세요.

[수집된 기사 정보]
{raw_text[:3500] if raw_text else "(검색된 내용 없음 — 아는 기사로 대체해주세요)"}
...
[정리 규칙]
- 위 기사 정보를 바탕으로 JSON 배열을 만들어주세요
- 기사 정보가 없거나 부족하면, 이 토론 주제와 관련해 실제로 존재하는 한국 뉴스 기사를 당신이 알고 있는 것으로 4개 추가해주세요
- summary: 초등학생도 이해할 수 있는 2~3문장 (어려운 말 풀어쓰기)
- evidence: 기사에서 근거로 쓸 수 있는 핵심 문장 1개 (구체적 사실·수치·발언 포함)
- why: 왜 이 문장이 근거가 되는지 1문장 (예: "신문이 직접 취재한 내용이라 믿을 수 있어요")
- source_label: 언론사명 + 날짜 (예: 연합뉴스, 2025.03.12)
- side: pro(학생 주장 지지) / con(반박) / neutral(중립)
- url: 그라운딩 출처에서 연결, 없으면 빈 문자열

코드블록(```) 없이 JSON 배열만 출력:
[
  {{
    "title": "기사 제목",
    "summary": "초등학생용 요약 2~3문장",
    "evidence": "핵심 근거 문장 1개",
    "why": "왜 근거가 되는지 1문장",
    "evidence_type": "뉴스기사",
    ...
    with st.spinner("📰 기사 내용을 정리하고 있어요..."):
        convert_result = call_gemini_json(convert_sys, "")

    if convert_result.startswith("__NO_KEY__"):
        render_no_key_warning(); return
    if convert_result.startswith("__ERROR__"):
        st.session_state.search_results = []
        st.session_state.search_error   = f"정리 오류: {convert_result}"
        return

    items = _parse_json_results(convert_result)

    # URL 보완: grounding sources 순서대로 채우기
    for i, item in enumerate(items):
        if not item.get("url"):
            for src in grounding_sources:
                if src.get("url") and src.get("url") not in [x.get("url") for x in items]:
    ...
    if items:
        ...
        st.session_state.search_results = []
        st.session_state.search_error   = f"기사 정리 실패. 원문 앞부분: {convert_result[:200]}"
