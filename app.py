...
def _render_final_speech(final_claim, final_reason):
    ...
    st.markdown("---")
    st.markdown("""
    <div style="background:#f0f9ff;border:2px solid #0ea5e9;border-radius:16px;
                padding:18px;margin-bottom:14px;">
        <div style="font-size:1rem;font-weight:900;color:#0369a1;margin-bottom:8px;">
            📥 활동 기록 저장하기
        </div>
        <div style="font-size:13px;color:#0369a1;line-height:1.7;">
            지금까지 작성한 모든 내용(주장·이유·피드백·반론·발표문)을
            TXT 파일로 저장할 수 있어요.<br>
            <b>선생님께 제출하거나 나중에 다시 볼 때 활용해봐요!</b>
        </div>
    </div>
    """, unsafe_allow_html=True)

    grade  = st.session_state.get("student_grade","0")
    cls    = st.session_state.get("student_class","0")
    num    = st.session_state.get("student_number","0")
    from datetime import datetime
    date_str = datetime.now().strftime("%Y%m%d")
    filename = f"HEAL_{grade}학년_{cls}반_{num}번_{date_str}.txt"

    txt_content = generate_txt_report()
    st.download_button(
        label="📥 활동 기록 TXT 다운로드",
...
[핵심 평가 원칙 — 반드시 지킬 것]
★ STEP 1: 주장-이유 일관성 먼저 판단
  - claimStance: 주장이 찬성인지 반대인지 판단
  - reasonStance: 이유가 찬성 측 논리인지 반대 측 논리인지 판단
  - 예1) 주장="해상풍력 건설 필요하다"(찬성) + 이유="어민 피해가 크다"(반대) → isConsistent=false
  - 예2) 주장="해상풍력 반대한다"(반대) + 이유="탄소 감축에 도움된다"(찬성) → isConsistent=false
  - 예3) 주장="해상풍력 필요하다"(찬성) + 이유="재생에너지로 탄소 줄어든다"(찬성) → isConsistent=true
  - isConsistent=false이면 reasonScore는 반드시 "노력바람"으로 설정
  - consistencyAlert: 불일치 시 "주장은 [찬성/반대]인데 이유는 [반대/찬성] 방향이에요. [구체적 설명]" 형식

★ STEP 2: 지침 기반 평가 (일관성 통과한 경우만 의미 있음)
...
{{
  ...
  "claimStance": "찬성 또는 반대 (주장 분석 결과)",
  "reasonStance": "찬성 또는 반대 (이유 분석 결과)",
  "isConsistent": false,
  "consistencyAlert": "불일치 시 설명, 일치 시 빈 문자열",
...
def render_student_info():
    """앱 최상단 — 학년/반/번호 입력 (한 번만)"""
    if st.session_state.student_info_saved:
        grade  = st.session_state.student_grade
        cls    = st.session_state.student_class
        num    = st.session_state.student_number
        st.markdown(f"""
        <div style="background:#e0f2fe;border-radius:12px;padding:8px 16px;
                    margin-bottom:10px;display:flex;align-items:center;
                    justify-content:space-between;">
            <span style="font-size:13px;color:#0369a1;font-weight:700;">
                🎒 {grade}학년 {cls}반 {num}번
            </span>
        </div>
        """, unsafe_allow_html=True)
        return

    with st.expander("🎒 시작 전에 학년·반·번호를 입력해요!", expanded=True):
        st.markdown("""
        <div style="font-size:13px;color:#475569;margin-bottom:10px;">
            이름은 적지 않아도 돼요. 선생님이 활동을 확인할 수 있도록
            학년·반·번호만 알려주세요!
        </div>
        """, unsafe_allow_html=True)
        c1, c2, c3, c4 = st.columns([1,1,1,2])
        with c1:
            grade = st.selectbox("학년", ["6"], label_visibility="visible")
        with c2:
            cls = st.number_input("반", min_value=1, max_value=20,
                                  value=1, step=1, label_visibility="visible")
        with c3:
            num = st.number_input("번호", min_value=1, max_value=40,
                                  value=1, step=1, label_visibility="visible")
        with c4:
            st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
            if st.button("✅ 확인!", use_container_width=True, key="btn_save_student"):
                st.session_state.student_grade       = str(grade)
                st.session_state.student_class       = str(int(cls))
                st.session_state.student_number      = str(int(num))
                st.session_state.student_info_saved  = True
                st.rerun()


def generate_txt_report() -> str:
    """학생 전체 활동 내역을 TXT로 생성"""
    from datetime import datetime
    now  = datetime.now().strftime("%Y-%m-%d %H:%M")
    grade = st.session_state.get("student_grade","?")
    cls   = st.session_state.get("student_class","?")
    num   = st.session_state.get("student_number","?")

    lines = []
    lines.append("=" * 60)
    lines.append("  H.E.A.L 프로젝트 — 학생 활동 기록")
    lines.append("=" * 60)
    lines.append(f"  학년/반/번호 : {grade}학년 {cls}반 {num}번")
    lines.append(f"  토론 주제    : {TOPIC}")
    lines.append(f"  저장 일시    : {now}")
    lines.append("=" * 60)

    # ── 1단계 ──
    lines.append("")
    lines.append("【 1단계: 주장 펼치기 】")
    lines.append("-" * 40)
    lines.append(f"[처음 쓴 주장]\n{st.session_state.get('claim','(없음)')}")
    lines.append(f"\n[처음 쓴 이유]\n{st.session_state.get('reason','(없음)')}")

    ana = st.session_state.get("ai_analysis") or {}
    if ana:
        lines.append(f"\n[AI 피드백]")
        lines.append(f"  주장 평가  : {ana.get('claimScore','')}")
        lines.append(f"  주장 칭찬  : {ana.get('claimPraise','')}")
        lines.append(f"  주장 개선  : {ana.get('claimImprove','')}")
        lines.append(f"  이유 평가  : {ana.get('reasonScore','')}")
        lines.append(f"  일관성     : {'✅ 일치' if ana.get('isConsistent',True) else '⚠️ 불일치 — ' + ana.get('consistencyAlert','')}")
        lines.append(f"  이유 칭찬  : {ana.get('reasonPraise','')}")
        lines.append(f"  이유 개선  : {ana.get('reasonImprove','')}")

    # 선택한 기사 근거
    sel_int = [s for s in st.session_state.get("selected_articles",[]) if isinstance(s,int)]
    sel_str = [s for s in st.session_state.get("selected_articles",[]) if isinstance(s,str)]
    if sel_int or sel_str:
        lines.append(f"\n[선택한 근거]")
        for sid in sel_int:
            art = next((a for a in ARTICLES if a["id"]==sid), None)
            if art:
                lines.append(f"  • {art['evidence']}")
                lines.append(f"    출처: {art['source']}, {art['date']}")
        for wid in sel_str:
            try:
                idx = int(wid.replace("web_",""))
                results = st.session_state.get("search_results",[])
                if idx < len(results):
                    item = results[idx]
                    memo = st.session_state.get(f"memo_web_{idx}","")
                    ev = memo or item.get("evidence","")
                    lines.append(f"  • {ev}")
                    lines.append(f"    출처: {item.get('source_label','')} | {item.get('url','')}")
            except Exception:
                pass
    ...
    lines.append("【 고쳐쓰기 】")
    lines.append("-" * 40)
    lines.append(f"[고쳐 쓴 주장]\n{st.session_state.get('revised_claim','(미작성)')}")
    lines.append(f"\n[고쳐 쓴 이유]\n{st.session_state.get('revised_reason','(미작성)')}")
    lines.append(f"\n[고쳐 쓴 근거]\n{st.session_state.get('revised_evidence','(미작성)')}")

    rfb = st.session_state.get("revision_feedback") or {}
    if rfb:
        lines.append(f"\n[고쳐쓰기 피드백]")
        lines.append(f"  전체 평가  : {rfb.get('overallPraise','')}")
        lines.append(f"  주장 변화  : {rfb.get('claimChange','')}")
        lines.append(f"  이유 변화  : {rfb.get('reasonChange','')}")
        lines.append(f"  근거 평가  : {rfb.get('evidenceChange','')}")
        if rfb.get('stillImprove'):
            lines.append(f"  아쉬운 점  : {rfb.get('stillImprove','')}")

    combined = st.session_state.get("combined_essay","")
    if combined:
        lines.append(f"\n[완성된 주장하는 글]\n{combined}")

    # ── 2단계 ──
    lines.append("")
    lines.append("【 2단계: 반론 펼치기 】")
    lines.append("-" * 40)
    lines.append(f"[① 인정]\n{st.session_state.get('rebuttal_admit','(미작성)')}")
    lines.append(f"\n[② 하지만+반론]\n{st.session_state.get('rebuttal_but','(미작성)')}")
    lines.append(f"\n[③ 근거]\n{st.session_state.get('rebuttal_evidence','(미작성)')}")
    lines.append(f"\n[완성된 반론문]\n{st.session_state.get('rebuttal','(미작성)')}")

    rfb2 = st.session_state.get("rebuttal_feedback") or {}
    if rfb2:
        lines.append(f"\n[반론 피드백]")
        lines.append(f"  구조 평가  : {rfb2.get('structurePraise','')}")
        lines.append(f"  반론 강도  : {rfb2.get('overall','')}")
        lines.append(f"  강화 팁    : {rfb2.get('tip','')}")

    # ── 3단계 ──
    lines.append("")
    lines.append("【 3단계: 주장 다지기 】")
    lines.append("-" * 40)

    # 선택한 최강 근거
    best_key = st.session_state.get("best_evidence_key","")
    best_reason = st.session_state.get("best_evidence_reason","")
    if best_key:
        # 라벨 찾기
        all_labels = []
        for a in ARTICLES:
            all_labels.append((f"a_{a['id']}", a['evidence'], a['source']))
        for i, item in enumerate(st.session_state.get("search_results",[])):
            memo = st.session_state.get(f"memo_web_{i}","")
            ev   = memo or item.get("evidence","")
            all_labels.append((f"w_{i}", ev, item.get("source_label","")))
        best = next((x for x in all_labels if x[0]==best_key), None)
        if best:
            lines.append(f"[선택한 최강 근거]\n  {best[1]}\n  출처: {best[2]}")
            lines.append(f"\n[왜 이 근거를 골랐나요]\n{best_reason}")

    lines.append(f"\n[최종 발표문]\n{st.session_state.get('final_speech','(미완성)')}")

    lines.append("")
    lines.append("=" * 60)
    lines.append("  ※ 이 파일은 H.E.A.L 프로젝트 AI 토론 도우미에서 자동 생성됐습니다.")
    lines.append("=" * 60)
