...
def render_progress(step: int):
    ...
    # step 0 = 주장펼치기, 0.5(고쳐쓰기는 step0 내부), 1 = 반론, 2 = 다지기
    # 진행 표시: step0→인덱스0 active, step1→인덱스2 active, step2→인덱스3 active
    active_map = {0: 0, 1: 2, 2: 3}
    active_idx = active_map.get(step, 0)
    # 고쳐쓰기는 1단계 내부에서 진행 → revised_claim 있으면 done
    ...
    for i, s in enumerate(labels):
        if i == 1:  # 고쳐쓰기
            ...
            cls, pre = "step-todo",   ""
        html += f'<div class="step-item {cls}">{pre}{s.replace(chr(10),"<br>")}</div>'
    html += "</div>"
...
def _render_rewrite_section():
    """피드백과 근거를 옆에 두고 고쳐쓰기 → 막히면 AI 힌트"""
    st.markdown("---")
    st.markdown("""
    <div style="background:linear-gradient(135deg,#fef9c3,#fde68a);border-radius:16px;
                padding:16px 20px;margin-bottom:16px;">
        <div style="font-size:1.1rem;font-weight:900;color:#92400e;">✍️ 고쳐쓰기 활동</div>
        <div style="font-size:0.85rem;color:#78350f;margin-top:4px;">
            AI 피드백과 찾은 근거를 참고해서 주장과 이유를 더 좋게 고쳐봐요!
        </div>
    </div>
    """, unsafe_allow_html=True)

    # 원본 + 피드백 나란히 보기
    col_orig, col_fb = st.columns([1, 1])
    with col_orig:
        st.markdown("**📄 처음 쓴 글**")
        ana = st.session_state.ai_analysis or {}
        score_badge = {
    ...
    with col_fb:
        st.markdown("**💡 개선 포인트**")
        claim_improve = ana.get("claimImprove","")
        reason_improve = ana.get("reasonImprove","")
        reason_deeper = ana.get("reasonDeeperQ","")
        ...
        sel_arts = [s for s in st.session_state.selected_articles if isinstance(s,int)]
        for sid in sel_arts:
            art = next((a for a in ARTICLES if a["id"]==sid), None)
            if art:
                memos.append(f"• {art['evidence']} ({art['source']})")

        st.markdown(f"""
        <div style="background:#fff7ed;border-radius:10px;padding:12px 14px;font-size:13px;
                    color:#92400e;line-height:1.8;margin-bottom:8px;">
            {"<b>주장 개선:</b> " + claim_improve + "<br>" if claim_improve else ""}
            {"<b>이유 개선:</b> " + reason_improve + "<br>" if reason_improve else ""}
            {"<b>생각해봐요:</b> " + reason_deeper if reason_deeper else ""}
        </div>
        """, unsafe_allow_html=True)

        if memos:
            st.markdown(f"""
            <div style="background:#f0fdf4;border-radius:10px;padding:12px 14px;font-size:13px;color:#166534;">
                <b>📌 내가 찾은 근거:</b><br>{"<br>".join(memos)}
            </div>
            """, unsafe_allow_html=True)
    ...
    # 고쳐쓰기 입력칸
    st.markdown("**✏️ 고쳐 쓴 주장**")
    revised_claim = st.text_area("고쳐쓴 주장", value=st.session_state.revised_claim,
        placeholder="피드백을 참고해서 주장을 더 명확하게 고쳐봐요!",
        height=80, label_visibility="collapsed", key="input_revised_claim")

    st.markdown("**✏️ 고쳐 쓴 이유**")
    revised_reason = st.text_area("고쳐쓴 이유", value=st.session_state.revised_reason,
        placeholder="근거를 활용해서 이유를 더 구체적으로 고쳐봐요!",
        height=100, label_visibility="collapsed", key="input_revised_reason")

    # 막힐 때 힌트 버튼
    col_hint, col_submit = st.columns([1, 1])
    with col_hint:
        if st.button("🆘 막혔어요! 예시 힌트 주세요", key="btn_hint"):
            st.session_state.show_hint = True
            _run_hint()

    with col_submit:
        can_submit = bool(revised_claim.strip() and revised_reason.strip())
        if st.button("✅ 고쳐쓰기 완료!", disabled=not can_submit, key="btn_revision_done"):
            st.session_state.revised_claim  = revised_claim
            st.session_state.revised_reason = revised_reason
            _run_revision_feedback(revised_claim, revised_reason)

    # 힌트 표시
    if st.session_state.show_hint and st.session_state.get("revision_hint"):
        ...
        <div style="background:#ede9fe;border-left:4px solid #8b5cf6;border-radius:12px;
            ...
            <b>주장 예시:</b> {hint.get("claimExample","")}<br>
            <b>이유 예시:</b> {hint.get("reasonExample","")}<br>
            <div style="margin-top:8px;font-size:12px;color:#6d28d9;">
                💡 이 예시를 그대로 쓰지 말고, 참고해서 나만의 문장으로 바꿔봐요!
            </div>
        </div>
        """, unsafe_allow_html=True)
...
학생이 주장과 이유를 고쳐쓰다가 막혔어요. 예시 힌트를 주세요.
...
def _run_revision_feedback(revised_claim: str, revised_reason: str):
    """고쳐쓴 글 vs 원본 비교 피드백"""
    sys_prompt = f"""초등학교 6학년 토론 선생님입니다.
...
def _render_revision_feedback():
    ...
    with col_b:
        st.markdown(f"""
        <div style="background:#fee2e2;border-radius:12px;padding:14px;font-size:13px;">
    ...
    with col_a:
        st.markdown(f"""
        <div style="background:#dcfce7;border-radius:12px;padding:14px;font-size:13px;">
    ...
    if st.button("➡️ 2단계: 반론하기로 가기!", key="btn_to_step2"):
