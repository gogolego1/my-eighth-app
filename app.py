...
def _run_web_search(claim: str, reason: str):
    """2단계 분리: ① Google Search로 원문 수집 → ② JSON 변환"""
    ana = st.session_state.ai_analysis or {}
    sq  = ana.get("searchQueries", {})
    en  = ana.get("evidenceNeed", {})

    q_stat   = sq.get("stat",   f"{reason} 통계")
    q_expert = sq.get("expert", f"{reason} 전문가 의견")
    q_case   = sq.get("case",   f"{reason} 사례")

    # ── STEP 1: Google Search로 실제 검색 ──
    search_prompt = f"""아래 토론 주제와 관련된 자료를 Google에서 검색해주세요.

토론 주제: {TOPIC}
학생 주장: {claim}
학생 이유: {reason}

다음 3가지 검색어로 각각 검색해서 실제 존재하는 자료를 찾아주세요:
① 통계/수치: "{q_stat}"
② 전문가/기관 의견: "{q_expert}"
③ 실제 사례: "{q_case}"

찾은 자료마다 아래 정보를 포함해서 답해주세요:
- 자료 제목
- 출처(기관명/언론사 + 연도)
- 핵심 내용 요약 (2~3문장)
- 가장 중요한 사실·수치·발언 1문장
- URL (있는 경우)
- 자료 유형: 통계/전문가의견/사례 중 하나
- 학생 주장 지지 여부: 지지/반대/중립 중 하나

총 5~6개 자료를 찾아주세요. 광고, 출처불명, 블로그 후기는 제외하세요."""

    with st.spinner("🌐 인터넷에서 통계·전문가 의견·사례를 검색하고 있어요..."):
    ...
    if raw_text.startswith("__ERROR__"):
        ...
        st.session_state.search_error = str(raw_text)
        return

    # ── STEP 2: 검색 결과를 JSON으로 변환 ──
    convert_prompt = f"""아래는 Google 검색으로 찾은 자료입니다.
이 내용을 초등학교 6학년이 이해할 수 있는 수준으로 정리해서 JSON 배열로 변환해주세요.
...
[변환 규칙]
- summary: 초등학생도 이해할 수 있는 2~3문장 (어려운 단어 풀어쓰기)
- evidence: 반드시 구체적 수치·사실·발언이 포함된 1문장 (가장 핵심적인 근거 문장)
- why: "이 문장이 왜 근거가 되는지" 초등학생 언어 1문장
  예시: "정부가 공식으로 발표한 숫자이기 때문에 믿을 수 있어요"
       "실제로 일어난 일이기 때문에 내 주장의 증거가 돼요"
       "전문가가 연구해서 나온 결과이기 때문에 믿을 수 있어요"
- evidence_type: 통계 / 전문가 의견 / 실제 사례 중 하나
- side: pro(학생 주장 지지) / con(반대) / neutral(중립)
- url: 위 URL 목록에서 가장 관련된 것 연결, 없으면 빈 문자열
...
[
  {{
    ...
    "source_label": "출처 기관·매체 + 연도",
    "side": "pro 또는 con 또는 neutral",
    "url": "URL 또는 빈 문자열"
  }}
]"""

    with st.spinner("📝 검색 결과를 쉽게 정리하고 있어요..."):
        convert_result = call_gemini_json(convert_prompt, "")

    if convert_result.startswith("__ERROR__"):
        st.session_state.search_results = []
        st.session_state.search_error = convert_result
        return

    try:
        clean = re.sub(r"```json|```|`", "", convert_result).strip()
        ...
        st.session_state.search_results = []
        st.session_state.search_error   = f"JSON 변환 실패: {e}\n원문: {convert_result[:300]}"
