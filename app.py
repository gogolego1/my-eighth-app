        ...
        with cols[idx]:
            if st.button(
                ...
                if accessible:
                    ...
                    # 고쳐쓰기 버튼(idx=1) 클릭 시 1단계로 스크롤 표시
                    if idx == 1:
...
def step1_rewrite():
    ...
    <div style="background:linear-gradient(135deg,#fef9c3,#fde68a);border-radius:18px;
        ...
        <div style="font-size:0.85rem;color:#78350f;margin-top:4px;">
            AI 피드백과 찾은 근거를 보면서 주장과 이유를 더 좋게 고쳐봐요!
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_r:
        ...
        # 일관성 문제가 있으면 먼저 표시
        consistency_alert = ana.get("consistencyAlert","")
        if consistency_alert:
            ...
            <div style="background:#fff1f2;border-radius:10px;padding:10px 14px;
                ...
                <b>🚨 일관성 문제:</b><br>{consistency_alert}
            </div>
            """, unsafe_allow_html=True)
        ...
        <div style="background:#fff7ed;border-radius:10px;padding:12px 14px;
            ...
            {"<b>💬 주장 개선:</b> " + claim_improve + "<br>" if claim_improve else "<b>💬 주장:</b> 잘 썼어요!<br>"}
            {"<b>🌿 이유 개선:</b> " + reason_improve + "<br>" if reason_improve else "<b>🌿 이유:</b> 잘 썼어요!<br>"}
            {"<b>🤔 더 생각해봐요:</b> " + reason_deeper if reason_deeper else ""}
        </div>
        """, unsafe_allow_html=True)
        ...
        sel_arts = [s for s in st.session_state.get("selected_articles",[]) if isinstance(s,int)]
        for sid in sel_arts:
        ...
        if memos:
            ...
            <div style="background:#f8fafc;border-radius:10px;padding:10px 14px;
                ...
                아직 찾은 근거가 없어요.<br>1단계로 돌아가서 기사를 찾아봐요.
            </div>
            """, unsafe_allow_html=True)
    ...
    st.markdown("**💬 고쳐 쓴 주장**")
    revised_claim = st.text_area("고쳐쓴 주장",
        value=st.session_state.revised_claim,
        placeholder="피드백을 참고해서 주장을 더 명확하게 고쳐봐요! 예) 해상풍력 발전단지 건설은 기후 위기를 막기 위해 반드시 필요하다.",
        height=90, label_visibility="collapsed", key="rw_claim")

    st.markdown("**🌿 고쳐 쓴 이유**")
    revised_reason = st.text_area("고쳐쓴 이유",
        value=st.session_state.revised_reason,
        placeholder="근거를 활용해서 이유를 더 구체적으로 고쳐봐요! 예) 왜냐하면 석탄 발전 대신 바람으로 전기를 만들면 이산화탄소가 크게 줄어들기 때문이다.",
        height=100, label_visibility="collapsed", key="rw_reason")
...
[핵심 평가 원칙 — 반드시 지킬 것]
★ 주장-이유 일관성 검사 (최우선):
  - 학생이 쓴 주장의 입장(찬성/반대)과 이유의 방향이 같아야 합니다.
  - 예: 주장이 "해상풍력 건설에 찬성" → 이유도 찬성 근거여야 함
  - 예: 주장이 "해상풍력 건설에 반대" → 이유도 반대 근거여야 함
  - 주장은 찬성인데 이유가 반대 입장이면 → reasonScore는 반드시 "노력바람"
  - 일관성이 없으면 reasonImprove에 반드시 "주장과 이유의 방향이 반대예요" 명시
...
[평가 등급 기준]
- 매우잘함: 일관성 OK + 지침 8개 이상 충족
- 잘함: 일관성 OK + 지침 5~7개 충족
- 노력바람: 일관성 불일치 OR 지침 4개 이하 충족

반드시 아래 JSON 형식으로만 출력하세요. 코드블록(```) 없이 JSON만 출력하세요.
{{
  "keywords": ["키워드1","키워드2","키워드3","키워드4"],

  "claimStance": "찬성 또는 반대 (학생 주장의 입장)",
  "reasonStance": "찬성 또는 반대 (학생 이유의 방향)",
  "isConsistent": true 또는 false,
  "consistencyAlert": "일관성 문제 설명 (일치하면 빈 문자열, 불일치면 구체적 설명)",

  "claimScore": "매우잘함 또는 잘함 또는 노력바람",
  "claimChecked": ["충족한 지침 번호"],
  "claimMissed":  ["부족한 지침 번호"],
...
def _render_analysis_result():
    ...
    # ── 일관성 경고 (최우선 표시) ──
    ana0 = st.session_state.ai_analysis or {}
    if not ana0.get("isConsistent", True):
        alert = ana0.get("consistencyAlert", "주장과 이유의 방향이 다를 수 있어요.")
        cs_stance = ana0.get("claimStance", "")
        rs_stance = ana0.get("reasonStance", "")
        st.markdown(f"""
        <div style="background:#fff1f2;border:2px solid #f87171;border-radius:14px;
            ...
            <div style="font-size:13px;color:#7f1d1d;line-height:1.8;">
                ...
                <b>💡 어떻게 고치면 좋을까요?</b><br>
                ① 주장을 이유에 맞게 바꾸거나<br>
                ② 이유를 주장에 맞게 바꿔야 해요.<br>
                아래 <b>고쳐쓰기</b> 단계에서 수정해봐요!
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def _render_selected_summary():
    ...
    if st.button("➡️ 2단계로 가기!", key="btn_to_step2_from_summary"):
