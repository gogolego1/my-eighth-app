...
def call_gemini_with_search(prompt: str) -> tuple[str, list]:
    """Google Search 그라운딩 활성화 호출 → (텍스트, 출처목록)"""
    api_key = get_api_key()
    if not api_key:
        return "__NO_KEY__", []
    try:
        ...
        response = model.generate_content(
            prompt,
            generation_config={"max_output_tokens": 2000, "temperature": 0.5},
        )
...
ARTICLES = [
    ...
    },
    {
        ...
        "summary": "완도-제주 해상경계 분쟁은 1996년부터 지금까지 해결되지 않고 있습니다. 바다는 이제 어업을 넘어 해상풍력·블루카본·해양관광이 집중되는 미래 자원 공간으로, 해역 경계가 불명확하면 인허가 지연과 법적 불확실성이 생겨 지역 경제에 큰 손실이 발생합니다.",
    ...
    },
    {
        ...
        "summary": "하나은행이 한국남동발전과 600MW 규모 완도금일해상풍력 발전단지 조성을 위한 MOU를 맺었습니다. 이 사업은 국내 최대 규모로, 생산 전력은 국가 AI 데이터센터와 호남권 첨단전략산업 전력 인프라로 활용될 예정입니다.",
...
# ── 주장·이유·근거 카드 ────────────────────────────────────────
def render_arg_card(label, css_class, icon, text, feedback=None, feedback_css=None, placeholder="아직 입력하지 않았어요"):
...
def _render_news_card(article: dict):
    ...
    border = "border:2px solid #0ea5e9;" if is_selected else "border:1.5px solid #e2e8f0;"
    side_badge = {"찬성":"🟢 찬성 측","반대":"🔴 반대 측","조건부":"🟡 조건부"}.get(article["side"],"")
    st.markdown(f"""
    <div class="news-card" style="{border}">
        <div class="news-source">📰 {article['source']} · {article['date']} &nbsp;
            <span style="background:#f1f5f9;padding:2px 8px;border-radius:99px;font-size:11px;">{side_badge}</span>
        </div>
    ...
    """, unsafe_allow_html=True)
    label = "✅ 선택됨!" if is_selected else "📌 근거로 사용하기"
    if st.button(label, key=f"sel_art_{sid}"):
        if is_selected: st.session_state.selected_articles.remove(sid)
        else:           st.session_state.selected_articles.append(sid)
        st.rerun()


# ── 웹 검색 결과 카드 ─────────────────────────────────────────
def _render_search_result_card(item: dict, idx: int):
    sid = f"web_{idx}"
    is_selected = sid in st.session_state.selected_articles
    border = "border:2px solid #0ea5e9;" if is_selected else ""
    side = item.get("side", "neutral")
    if side == "pro":
        card_cls, badge_html = "search-card search-card-pro", '<span class="search-badge-pro">🟢 찬성 측 근거</span>'
    elif side == "con":
        card_cls, badge_html = "search-card search-card-con", '<span class="search-badge-con">🔴 반대 측 근거</span>'
    else:
        card_cls, badge_html = "search-card", '<span class="search-badge-neu">🔵 참고 자료</span>'

    url = item.get("url", "#")
    url_html = f'<a href="{url}" target="_blank" class="source-link">🔗 원문 보러가기 ↗</a>' if url != "#" else ""
    ...
    label = "✅ 선택됨!" if is_selected else "📌 근거로 사용하기"
    if st.button(label, key=f"sel_web_{idx}"):
        if is_selected: st.session_state.selected_articles.remove(sid)
        else:           st.session_state.selected_articles.append(sid)
        st.rerun()
...
def _run_analysis(claim: str, reason: str):
    system = """당신은 초등학교 6학년 학생을 돕는 친절한 국어·사회 선생님입니다.
학생의 주장과 이유를 꼼꼼하게 분석하고, 반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트는 절대 포함하지 마세요.
{
  "keywords": ["키워드1", "키워드2", "키워드3", "키워드4"],
  "claimFeedback": "주장에 대한 구체적 피드백 2~3문장. ①주장이 명확한지 ②찬반 입장이 분명한지 평가하고 칭찬+개선점 제시. 초등학생 언어.",
  "claimScore": "상/중/하 중 하나",
  "reasonFeedback": "이유에 대한 구체적 피드백 2~3문장. ①주장과 연결되는지 ②왜 그렇게 생각하는지 설명이 충분한지 평가하고 칭찬+개선점 제시. 초등학생 언어.",
  "reasonScore": "상/중/하 중 하나",
  "evidenceHint": "이 이유를 뒷받침하려면 어떤 종류의 근거(통계, 전문가 의견, 사례 등)가 필요한지 구체적으로 3문장 안내. 초등학생 언어.",
  "searchQueries": ["검색어1", "검색어2", "검색어3"]
}
searchQueries는 학생의 이유를 뒷받침할 실제 자료를 찾기 위한 한국어 검색어 3개를 만들어주세요."""

    with st.spinner("✏️ AI 선생님이 주장과 이유를 꼼꼼히 읽고 있어요..."):
        result = call_gemini_json(system, f"주장: {claim}\n이유: {reason}")

    if result.startswith("__NO_KEY__"):
        render_no_key_warning(); return
    if result.startswith("__ERROR__"):
        st.error(f"AI 오류: {result}"); return
    ...
    try:
        ...
        parsed = {
            ...
            "claimFeedback": "주장을 잘 썼어요! 찬성인지 반대인지 입장이 분명하게 드러나 있어요.",
            "claimScore": "상",
            "reasonFeedback": "이유도 잘 쓰여 있어요. 왜 그렇게 생각하는지 더 자세히 설명하면 더 좋아요.",
            "reasonScore": "중",
            "evidenceHint": "통계 자료나 전문 기관의 보고서를 찾아보세요. 숫자나 연구 결과가 있으면 더 믿을 수 있어요.",
            "searchQueries": ["해상풍력 기후위기 효과", "재생에너지 탄소감축 통계", "해상풍력 어민 피해 사례"],
        }
...
def _run_web_search(claim: str, reason: str):
    queries = st.session_state.ai_analysis.get("searchQueries", []) if st.session_state.ai_analysis else []
    query_str = ", ".join(queries) if queries else f"{reason} 관련 자료"

    prompt = f"""초등학교 6학년 학생이 아래 토론 주제로 글을 쓰고 있어요.

토론 주제: {TOPIC}
학생의 주장: {claim}
학생의 이유: {reason}

이 이유를 뒷받침하거나 반박할 수 있는 실제 자료(뉴스 기사, 통계, 연구 결과, 정부 발표 등)를 인터넷에서 검색해서 찾아주세요.
검색 키워드: {query_str}
...
[
  {{
    ...
    "evidence": "근거로 쓸 수 있는 핵심 문장 1개 (통계, 사실, 전문가 의견 등)",
    "why": "왜 이 문장이 근거가 될 수 있는지 1문장 설명 (초등학생 언어)",
    "source_label": "출처 기관/매체명 + 연도",
    "side": "pro 또는 con 또는 neutral (학생 주장 기준으로 찬성이면 pro, 반대면 con)",
    "url": "실제 URL (없으면 빈 문자열)"
  }}
]
3~5개의 자료를 찾아주세요. JSON 배열만 반환하세요."""

    with st.spinner("🌐 인터넷에서 관련 자료를 검색하고 있어요..."):
...
def _render_analysis_result():
    ...
    score_map = {"상": "🌟 아주 잘했어요!", "중": "👍 잘했어요!", "하": "💪 조금 더 다듬어봐요!"}

    # 주장 카드
    claim_score = score_map.get(ana.get("claimScore", "중"), "")
    claim_fb = f"<b>{claim_score}</b><br>{ana.get('claimFeedback','')}"
    render_arg_card("💬 주장", "arg-claim", "💬",
                    st.session_state.claim, claim_fb, "fb-claim")

    # 이유 카드
    reason_score = score_map.get(ana.get("reasonScore", "중"), "")
    reason_fb = f"<b>{reason_score}</b><br>{ana.get('reasonFeedback','')}"
    render_arg_card("🌿 이유", "arg-reason", "🌿",
                    st.session_state.reason, reason_fb, "fb-reason")

    # 근거 힌트 카드
    ev_hint = ana.get("evidenceHint", "전문 자료에서 사실과 수치를 찾아보세요.")
    st.markdown(f"""
    <div class="arg-evidence">
        <div class="arg-label">📌 근거 (아직 찾는 중!)</div>
        <div class="arg-text" style="color:#c2410c;">근거는 내 생각이 아닌 <b>객관적인 자료</b>예요! 아래에서 찾아보세요.</div>
        <div class="fb-box fb-evidence" style="margin-top:8px;">
            <div class="fb-title">💡 어떤 근거를 찾으면 좋을까요?</div>
            {ev_hint}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if not results:
            ...
            st.markdown(f"""
            <div style="background:#f0fdf4;border-radius:12px;padding:10px 14px;margin-bottom:12px;font-size:13px;color:#166534;font-weight:600;">
                🌐 인터넷에서 <b>{len(results)}개</b>의 관련 자료를 찾았어요! 근거로 쓸 문장을 골라봐요.
            </div>
            """, unsafe_allow_html=True)
            for i, item in enumerate(results):
                _render_search_result_card(item, i)
...
def _render_selected_summary():
    sel_article_ids = [s for s in st.session_state.selected_articles if isinstance(s, int)]
    sel_web_ids     = [s for s in st.session_state.selected_articles if isinstance(s, str)]

    all_selected = sel_article_ids or sel_web_ids
    if not all_selected:
        return
    ...
    for wid in sel_web_ids:
        idx = int(wid.replace("web_", ""))
        results = st.session_state.get("search_results", [])
        if idx < len(results):
            item = results[idx]
            url = item.get("url", "#")
            link_html = f'<a href="{url}" target="_blank" style="font-size:12px;color:#0284c7;">🔗 출처 보기</a>' if url and url != "#" else ""
            st.markdown(f"""
...
def _run_rebuttal(opposite: str):
    system = """초등학교 6학년 토론 선생님입니다. 반드시 JSON으로만 응답하세요.
{
  "question": "상대방이 왜 그렇게 생각했을지 추측하는 질문 (쉬운 말, 1문장)",
  "rebuttalHint": "어떻게 반론하면 좋을지 힌트 2문장 (쉬운 말)",
  "strongerPoint": "내 주장을 더 강하게 만드는 방법 1문장",
  "weakPoint": "내 주장의 약한 점과 보완 방법 1문장"
}"""
    ...
    try:
        ...
        parsed = {"question":"상대방은 어떤 걱정이 있었을까요?","rebuttalHint":"상대 주장의 약점을 찾아봐요.","strongerPoint":"통계나 전문가 의견을 넣으면 더 강해요.","weakPoint":"내 주장도 약한 부분이 있는지 생각해봐요."}
...
def _render_rebuttal_result():
    ...
    with c1:
        st.markdown(f"""<div style="background:#eff6ff;border-radius:14px;padding:14px;border:1.5px solid #bae6fd;">
            ...
            <div style="font-size:14px;color:#1e40af;line-height:1.6;">{ana['question']}</div></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div style="background:#f0fdf4;border-radius:14px;padding:14px;border:1.5px solid #86efac;">
            ...
            <div style="font-size:14px;color:#14532d;line-height:1.6;">{ana['rebuttalHint']}</div></div>""", unsafe_allow_html=True)
    c3, c4 = st.columns(2)
    with c3:
        st.markdown(f"""<div style="background:#fef9c3;border-radius:14px;padding:14px;border:1.5px solid #fde047;margin-top:8px;">
            ...
            <div style="font-size:14px;color:#78350f;line-height:1.6;">{ana['strongerPoint']}</div></div>""", unsafe_allow_html=True)
    with c4:
        st.markdown(f"""<div style="background:#fff1f2;border-radius:14px;padding:14px;border:1.5px solid #fecdd3;margin-top:8px;">
            ...
            <div style="font-size:14px;color:#881337;line-height:1.6;">{ana.get('weakPoint','내 주장도 다시 한번 점검해봐요.')}</div></div>""", unsafe_allow_html=True)
...
def step3():
    ...
    st.markdown("#### 📌 가장 강한 근거를 골라봐요!")
    all_evidence = []
    for a in ARTICLES:
        all_evidence.append({"label": f"[{a['source']}] {a['evidence'][:60]}...", "key": f"a_{a['id']}", "text": a['evidence'], "source": a['source']})
    for i, item in enumerate(st.session_state.get("search_results",[])):
        all_evidence.append({"label": f"[인터넷] {item.get('evidence','')[:60]}...", "key": f"w_{i}", "text": item.get('evidence',''), "source": item.get('source_label','인터넷')})
    ...
    if st.button("✍️ 최종 발표문 만들기!", disabled=not can_gen, key="btn_speech"):
        _run_speech(all_evidence)

    if st.session_state.final_speech:
        _render_final_speech()


def _run_speech(all_evidence: list):
    ev_texts = []
    for ekey in st.session_state.selected_evidence:
        item = next((e for e in all_evidence if e["key"] == ekey), None)
        if item: ev_texts.append(f"- {item['text']} (출처: {item['source']})")

    system = """초등학교 6학년 토론 선생님입니다.
학생의 주장·이유·근거로 발표문을 써주세요.
