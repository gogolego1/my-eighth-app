...
학생의 주장과 이유를 꼼꼼하게 분석하고, 반드시 아래 JSON 형식으로만 응답하세요. 코드블록(```) 없이 JSON만 출력하세요.
{
  "keywords": ["키워드1","키워드2","키워드3","키워드4"],
  "claimFeedback": "주장 피드백 2~3문장: 찬반 입장 명확성 평가 + 칭찬 + 개선점. 초등학생 언어.",
  "claimScore": "상 또는 중 또는 하",
  "reasonFeedback": "이유 피드백 2~3문장: 주장과의 연결성·충분성 평가 + 칭찬 + 개선점. 초등학생 언어.",
  "reasonScore": "상 또는 중 또는 하",
  "evidenceHint": "어떤 근거(통계·전문가 의견·사례)가 필요한지 3문장 안내. 초등학생 언어.",
  "searchQueries": ["검색어1","검색어2","검색어3"]
}
searchQueries는 이유를 뒷받침할 실제 한국어 검색어 3개를 만들어주세요."""

    with st.spinner("✏️ AI 선생님이 주장과 이유를 읽고 있어요..."):
    ...
    try:
        ...
        parsed = {
            ...
            "claimFeedback": "주장이 명확하게 잘 작성되었어요! 찬성인지 반대인지 입장이 뚜렷이 드러나 있어요.",
            "claimScore": "상",
            "reasonFeedback": "이유도 잘 쓰여 있어요. 왜 그렇게 생각하는지 조금 더 자세히 설명하면 더 좋아요.",
            "reasonScore": "중",
            "evidenceHint": "통계 자료나 전문 기관의 보고서를 찾아보세요. 숫자나 연구 결과가 있으면 더 믿을 수 있어요.",
            "searchQueries": ["해상풍력 기후위기 탄소감축 효과","재생에너지 확대 필요성 통계","해상풍력 어민 피해 사례"],
        }
...
def _run_web_search(claim: str, reason: str):
    ...
    query_str = ", ".join(queries) if queries else reason

    prompt = f"""초등학교 6학년 학생이 토론 글을 쓰고 있어요.
...
위 이유를 뒷받침하거나 반박할 수 있는 실제 자료(뉴스, 통계, 연구, 정부 발표)를 Google에서 검색해서 찾아주세요.

아래 JSON 배열 형식으로만 응답하세요. 코드블록(```) 없이 JSON 배열만 출력하세요.
[
  {{
    "title": "자료 제목",
    "summary": "초등학생도 이해할 수 있는 2~3문장 요약",
    "evidence": "근거로 쓸 수 있는 핵심 문장 1개 (통계·사실·전문가 의견 등)",
    "why": "왜 이 문장이 근거가 될 수 있는지 1문장 (초등학생 언어)",
...
def _render_analysis_result():
    ...
    claim_fb  = f"<b>{score_map.get(ana.get('claimScore','중'))}</b><br>{ana.get('claimFeedback','')}"
    reason_fb = f"<b>{score_map.get(ana.get('reasonScore','중'))}</b><br>{ana.get('reasonFeedback','')}"
    ...
    with tab2:
        ...
        if not results:
            ...
            st.markdown(f'<div style="background:#f0fdf4;border-radius:12px;padding:10px 14px;margin-bottom:12px;font-size:13px;color:#166534;font-weight:600;">🌐 인터넷에서 <b>{len(results)}개</b>의 관련 자료를 찾았어요!</div>', unsafe_allow_html=True)
